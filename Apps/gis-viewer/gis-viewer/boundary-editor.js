...
var BoundaryEditor = (function() {
    ...
    function _(cesiumWidget) {
        this._keys = {'D_KEY' : 68, 'Alt' : 18, 'Ctrl' : 17}
        this._scene = cesiumWidget.scene;
        this._tooltip = createTooltip(cesiumWidget.container);
        this._surfaces = [];
        

        this.initialiseKeyboardHandlers();
        this.initialiseMouseHandlers();
        this.enhancePrimitives();

    }
    
    _.prototype.initialiseKeyboardHandlers = function() {
        var _self = this;
        var key = this._keys
        ...
        window.onkeydown = (function (event){
            ...
            if (!_self.isDDown() && keyId == key.D_KEY){
                _self.setDKeyDown(true);
                console.log(_self._DKeyDown);
            }
        });
        ...
        window.onkeyup = (function (event){
            ...
            var keyId = (window.event) ? event.keyCode : e.keyCode;
            if (_self.isDDown() && keyId == key.D_KEY){
                _self.setDKeyDown(false);
                console.log(_self._DKeyDown);
            }
        });
    }
    
    _.prototype.setDKeyDown = function(pressed){
        this._DKeyDown = pressed;
    }
    
    _.prototype.isDDown = function(){
        return this._DKeyDown;
    }
    

    _.prototype.initialiseMouseHandlers = function() {
    ...
    _.prototype.registerEditableShape = function(surface) {
        ...
        setListener(surface, 'mouseMove', function(position) {
            ...
            if(!surface._editMode && !_self.isDDown()) {
                _self._tooltip.showAt(position, "Click to edit this shape");
            } else if (_self.isDDown()){
                _self._tooltip.showAt(position, "Click to delete this shape");
            }
        });
    ...
    _.prototype.removePrimitive = function(surface) {
        var primitives = this._scene.getPrimitives();
        for(var index = 0; index < this._surfaces.length; index++) {
            if (surface == this._surfaces[index]){
                this._surfaces.splice(index,1);
                surface.setEditMode(false);
                primitives.remove(surface); 
            }
        }
    }
    ...
    _.DrawHelperWidget = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var drawOptions = {
                ...
                removeIcon: "./img/gliphicons_097_vector_path_pointer.png",
            ...
            addIcon('remove', options.removeIcon, 'Click to remove Boundaries', function() {
                drawHelper.startDrawingCircle({
                    callback: function(center, radius) {
                        _self.executeListeners({name: 'circleCreated', center: center, radius: radius});
                    }
